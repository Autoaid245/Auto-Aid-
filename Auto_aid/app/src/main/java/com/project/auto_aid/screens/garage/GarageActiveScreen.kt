...
fun GarageActiveScreen(
    ...
    val messages = remember { mutableStateListOf<GarageChatUiMessage>() }

    var socket by remember { mutableStateOf<Socket?>(null) }
    var chatConnected by remember { mutableStateOf(false) }
    var joinedOnce by remember { mutableStateOf(false) }

    val canChat = remember(status) { garageCanChatForStatus(status) }
    ...
    LaunchedEffect(requestId) {
        ...
        while (isActive) {
            try {
                ...
                if (garageIsTerminalStatus(status)) break
    ...
    Column(
        ...
        Text(
            text = garageDisplayStatusText(status),
...
private fun garageDisplayStatusText(status: String): String {
    return when (status.lowercase()) {
        ...
        "completed" -> "Garage service completed ✅"
        "cancelled" -> "Request cancelled ❌"
        else -> "Status: $status"
    }
}

private fun garageCanChatForStatus(status: String): Boolean {
...
private fun garageShouldShowProviderCard(status: String): Boolean {
    return status.lowercase() in listOf(
        "assigned",
        "mechanic_assigned",
        "driver_on_the_way",
...
private fun garageIsTerminalStatus(status: String): Boolean {
    return status.lowercase() in listOf("completed", "cancelled")
}

private data class GarageChatUiMessage(
    val id: String,
    val sender: String,
    val text: String,
...
private fun JSONObject.toUi(): GarageChatUiMessage {
    ...
    return GarageChatUiMessage(id, sender, text, createdAt)
}

@Composable
private fun GarageChatBubble(m: GarageChatUiMessage) {
